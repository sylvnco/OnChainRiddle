import {
	useReadOnchainRiddle,
	useWriteOnchainRiddleSubmitAnswer,
...
const RiddleForm = ({ isConnected, address }: RiddleFormType) => {
	...
	const {
		isLoading: isRiddleWinnerLoading,
		data: riddleWinner,
	...
	const { writeContractAsync, data: hash } =
		useWriteOnchainRiddleSubmitAnswer();
	...
	useWatchOnchainRiddleAnswerAttemptEvent({
		...
		enabled:
			riddle !== null && riddle !== undefined && riddleWinner === zeroAddress,
	});
	...
	if (!isRiddleWinnerLoading && riddleWinner !== zeroAddress) {
		return (
			<div className="flex flex-col h-full items-center justify-center gap-8">
				<p>
					Riddle already solved by{" "}
					{riddleWinner == address ? "you" : riddleWinner} !
				</p>
	...
	return (
		<div className="flex flex-col h-full items-center md:justify-center gap-24 md:gap-8 pt-24 md:pt-0">
			{isLoading ? (
				...
				<h1 className="text-center text-lg md:text-5xl font-bold max-w-5xl leading-tight">
					{riddle}
				</h1>
			)}
